...
export async function POST() {
  ...
  try {
    ...
    // 1. Tenta buscar ou gerar conexão na Evolution API
    try {
      ...
      console.warn('Instância não encontrada para connect, tentando criar...', e);
    }

    // 2. Se não conectou, cria a instância
    if (!qrCodeBase64) {
    ...
    // 3. Fallback visual para demonstração caso a Evolution ainda esteja iniciando
    if (!qrCodeBase64) {
